...
class logreglayer(nn.Module):
    def __init__(self,dims):
        ...
        self.w= torch.nn.Parameter(torch.randn((dims,1), requires_grad=True))
    ...
    def forward(self,x):
        ...
        return torch.sigmoid(torch.mm(x,self.w)+ self.bias)
...
def run():
    ...
    #Tensordataset
    #TODO
    dtr= torch.utils.data.TensorDataset(torch.Tensor(xtr),torch.Tensor(ytr))
    dv=torch.utils.data.TensorDataset(torch.Tensor(xv), torch.Tensor(yv))
    ...
    #TODO
    dte=torch.utils.data.TensorDataset(torch.Tensor(xt), torch.Tensor(yt))
