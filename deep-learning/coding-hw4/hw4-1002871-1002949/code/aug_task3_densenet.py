...
def main():
    transform = transforms.Compose([transforms.CenterCrop(330), transforms.ToTensor(), transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])])

    dataset = ImageNetDataset('imagenet/imagespart/', 'imagenet/data.csv', crop_size=330, transform=transform)
    dataset_loader = DataLoader(dataset, batch_size=config.BATCH_SIZE, shuffle=True, num_workers=config.NUM_WORKS)

    model = models.densenet121(pretrained=True).to(config.device)
    model.features.pool0 = nn.MaxPool2d(kernel_size=3, stride=2, padding=1, ceil_mode=False)
    model.features.transition1.pool = nn.AvgPool2d(kernel_size=2, stride=2, padding=0)
    model.features.transition1.pool = nn.AvgPool2d(kernel_size=2, stride=2, padding=0)
    model.features.transition1.pool = nn.AvgPool2d(kernel_size=2, stride=2, padding=0)
    model.eval()
